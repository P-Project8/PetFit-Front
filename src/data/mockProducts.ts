// Import Product type and real data from products.ts
import { products as realProducts, type Product } from './products';

// Legacy mock products (kept for backward compatibility)
// const legacyMockProducts: Product[] = [
//   {
//     id: 1,
//     name: '강아지 한복',
//     price: 69000,
//     imageUrl: '/src/assets/images/banner1.jpg',
//     description: '추석, 설날을 기념해보세요',
//     category: 'one-piece',
//     isCarousel: true,
//     isNew: true,
//   },
//   {
//     id: 2,
//     name: '겨자 후드티',
//     price: 38000,
//     imageUrl: '/src/assets/images/banner2.jpg',
//     description: '캐주얼한 느낌의 후드',
//     category: 'outer',
//     isHot: true,
//     isCarousel: true,
//   },
//   {
//     id: 3,
//     name: '당근 캐릭터 나시',
//     price: 29000,
//     imageUrl: '/src/assets/images/banner3.jpg',
//     description: '귀여운 당근이 포인트',
//     category: 'top',
//     isCarousel: true,
//     isLike: true,
//   },
//   {
//     id: 4,
//     name: '곰돌이 방수복',
//     price: 49000,
//     discountRate: 50,
//     imageUrl: '/src/assets/images/banner4.jpg',
//     description: '비 오는 날 걱정 마세요 :)',
//     category: 'outer',
//     isCarousel: true,
//     isSale: true,
//     isLike: true,
//   },
//   {
//     id: 5,
//     name: '알록달록 후드티',
//     price: 42000,
//     imageUrl: '/src/assets/images/banner5.jpg',
//     description: '눈에 띌 수 밖에 없는 귀여움',
//     category: 'top',
//     isCarousel: true,
//     isHot: true,
//     isNew: true,
//   },
//   {
//     id: 6,
//     name: '체크 머플러',
//     price: 18000,
//     category: 'muffler',
//     isLike: true,
//     discountRate: 30,
//     isSale: true,
//     isHot: true,
//   },
//   {
//     id: 7,
//     name: '방한 부츠',
//     price: 59000,
//     category: 'shoes',
//     discountRate: 70,
//     isSale: true,
//     isHot: true,
//   },
//   {
//     id: 8,
//     name: '리본 목걸이',
//     price: 12000,
//     category: 'accessory',
//     isLike: true,
//   },
//   {
//     id: 9,
//     name: '원피스 드레스',
//     price: 52000,
//     category: 'one-piece',
//     isNew: true,
//   },
//   {
//     id: 10,
//     name: '패딩 점퍼',
//     price: 85000,
//     category: 'outer',
//     isHot: true,
//     isLike: true,
//   },
//   {
//     id: 11,
//     name: '니트 스웨터',
//     price: 45000,
//     category: 'top',
//   },
//   {
//     id: 12,
//     name: '레인코트',
//     price: 69000,
//     discountRate: 50,
//     category: 'outer',
//     isSale: true,
//     isLike: true,
//   },
//   {
//     id: 13,
//     name: '스트라이프 티셔츠',
//     price: 33000,
//     category: 'top',
//     isNew: true,
//   },
//   {
//     id: 14,
//     name: '데님 재킷',
//     price: 75000,
//     category: 'outer',
//   },
//   {
//     id: 15,
//     name: '플리스 조끼',
//     price: 49000,
//     discountRate: 50,
//     category: 'outer',
//     isSale: true,
//   },
//   {
//     id: 16,
//     name: '하트 니트',
//     price: 46000,
//     category: 'top',
//     isHot: true,
//   },
//   {
//     id: 17,
//     name: '캐시미어 목도리',
//     price: 22000,
//     category: 'muffler',
//   },
//   {
//     id: 18,
//     name: '운동화',
//     price: 59000,
//     category: 'shoes',
//   },
//   {
//     id: 19,
//     name: '벨벳 리본',
//     price: 9000,
//     category: 'accessory',
//   },
//   {
//     id: 20,
//     name: '튤 원피스',
//     price: 48000,
//     category: 'one-piece',
//   },
//   {
//     id: 21,
//     name: '패턴 스웨터',
//     price: 39000,
//     discountRate: 40,
//     category: 'top',
//     isSale: true,
//   },
//   {
//     id: 22,
//     name: '베이직 후드',
//     price: 38000,
//     category: 'top',
//     isNew: true,
//     isLike: true,
//   },
//   {
//     id: 23,
//     name: '양털 조끼',
//     price: 55000,
//     category: 'outer',
//     isNew: true,
//   },
//   {
//     id: 24,
//     name: '레인부츠',
//     price: 49000,
//     category: 'shoes',
//     isLike: true,
//   },
//   {
//     id: 25,
//     name: '모자',
//     price: 15000,
//     category: 'etc',
//   },
//   {
//     id: 26,
//     name: '양말 세트',
//     price: 10000,
//     category: 'etc',
//     isLike: true,
//   },
//   {
//     id: 27,
//     name: '배낭',
//     price: 39000,
//     category: 'etc',
//     isLike: true,
//   },
// ];

// Export real products from CSV data
export const mockProducts = realProducts;

// Also export the Product type for convenience
export type { Product };
